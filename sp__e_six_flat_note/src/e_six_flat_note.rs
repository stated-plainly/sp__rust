...
impl tKeyboardFriendlyName for eSixFlatNote {
...
impl TryFrom<(eNoteLetter, eNoteModifier)> for eSixFlatNote {
    ...
    fn try_from(value: (eNoteLetter, eNoteModifier)) -> Result<Self, Self::Error> {
        match value {
            ...
            // Failure
            (note, modifier) => Err(Box::new(sImpossibleOperationError::new(format!("{}{} cannot be converted to a 6♭ note", note, modifier).as_str()))),
        }
    }
}
